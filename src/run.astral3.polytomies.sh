...
## Usage run.astral3.polytomies.sh -g <.genetrees> -s <species .mapfile>
...
if [ $single -eq 0 ]
then 	
	java -Xmx1G -jar ~/bin/ASTRAL_v5.6.3/astral.5.6.3.jar -i $genetrees -a ${mapspec} -o ${outname}.spectree -t 10 2> ${outname}.log
fi 

## Run ASTRAL on single-individual dataset using full annotation (-t 2)
if [ $single -eq 1 ]
then
	java -Xmx1G -jar ~/bin/ASTRAL_v5.6.3/astral.5.6.3.jar -i $genetrees -o ${outname}.single.spectree -t 10 2> ${outname}.single.log
fi
