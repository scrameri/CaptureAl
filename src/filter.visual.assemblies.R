...
## Usage: filter.visual.assemblies.R <sfile> <stats> <refseqs> <min.pregion=0.3> <min.pcontig=0.5> <min.ptaxa=0.3> <max.ncontigs=1> <min.bestscore.norm=1> <min.taln=50> <min.tfrac=0.2> <min.bestscore=1> <min.bestlength=1> <min.frac=0.9>
...
if (! length(args) %in% c(3:13)) {
  stop("3 arguments needed (13 taken):
       REQUIRED
       ...
       4) <min.pregion|NUM>:        minimum fraction of regions recovered in a sample (i.e., sample has at least 1 
                                    successfully aligned and passed contig in <min.pregion>*nregions regions) [DEFAULT: 0.3]

       5) <min.pcontig|NUM>:        minimum median contig length relative to target length [DEFAULT: 0.5] 
       
       # region filtering criteria (capture sensitivity)
       6) <min.ptaxa|NUM>:          minimum fraction of samples recovered in a region (i.e., region has at least 1 
                                    successfully aligned and passed contig in <min.ptaxa>*nsamples samples) [DEFAULT: 0.3]
                                    
       # region filtering criteria (capture specificity)
       7) <max.ncontigs|NUM>:       maximum number of (non-zero) contigs per region [DEFAULT: 1]
       8) <min.bestscore.norm|NUM>: minimum normalized alignment score (i.e., raw score / target alignment length) 
                                    of best alignment (filters for specificity) [DEFAULT: 1]
       9) <min.taln|NUM>:           minimum length of best alignment (in reference) [DEFAULT: 50]
       10) <min.tfrac|NUM>:          minimum fraction of best alignment (in reference) [DEFAULT: 0.2]
       11) <min.bestscore|NUM>:     minimum raw score of best alignment [DEFAULT: 1]
       12) <min.bestlength|NUM>:    minimum length of best aligned contig [DEFAULT: 1]
       13) <min.frac|NUM>:          minimum fraction of samples conforming to the absolute filtering criteria 6-11
                                    (i.e., regions must meet criteria 6-11 in (100*<min.frac>)% of considered samples,
...
min.pcontig <- as.numeric(as.character(args[5]))
if (is.na(min.pcontig)) min.pcontig <- 0.5

min.ptaxa <- as.numeric(as.character(args[6]))
if (is.na(min.ptaxa)) min.ptaxa <- 0.3
max.ncontigs <- as.numeric(as.character(args[7]))
if (is.na(max.ncontigs)) max.ncontigs <- 1
min.bestscore.norm <- as.numeric(as.character(args[8]))
if (is.na(min.bestscore.norm)) min.bestscore.norm <- 1
min.taln <- as.numeric(as.character(args[9]))
if (is.na(min.taln)) min.taln <- 50
min.tfrac <- as.numeric(as.character(args[10]))
if (is.na(min.tfrac)) min.tfrac <- 0.2
min.bestscore <- as.numeric(as.character(args[11]))
if (is.na(min.bestscore)) min.bestscore <- 1
min.bestlength <- as.numeric(as.character(args[12]))
if (is.na(min.bestlength)) min.bestlength <- 1
min.frac <- as.numeric(args[13])
...
# sfile = "mapfile.txt"
# stats = "loci_stats.txt"
# refseqs = "reference.fasta"

# min.pregion <- 0.3      # minimum proportion of regions with non-zero contigs (filters samples)
# min.pcontig <- 0.5      # minimum median contig length relative to target length (filters samples)
# min.ptaxa <- 0.3        # minimum proportion of samples with non-zero contigs (filters regions)
# max.ncontigs <- 1       # maximum number of non-zero contigs (filters regions)
# min.bestscore.norm <- 1 # minimum best normalized alignment score (filters regions)
# min.taln <- 50          # minimum alignment length (filters regions)
# min.tfrac <- 0.2        # minimum alignment length / target length (filters regions)
# min.bestscore <- 1      # minimum best alignment score (filters regions)
# min.bestlength <- 1     # minimum best contig length (filters regions)
# min.frac <- 0.9         # minimum fraction of conforming samples (filters regions)


## Additional arguments
# ouput paths
suffix <- paste(min.pregion, min.pcontig, min.ptaxa, max.ncontigs, min.bestscore.norm, min.taln, min.frac, sep = "-")
outpdf = paste0("regions_stats-", suffix, ".pdf")    # output .pdf (visualization of alignment stats)
outtxt1 = paste0("regions_stats-", suffix, ".txt")   # output .txt (merged alignment stats)
outtxt2 = paste0("regions_kept-", suffix, ".txt")    # filtered regions
outtxt3 = paste0("taxa_kept-", min.pregion, "-", min.pcontig, ".txt")  # filtered samples
logtxt = paste0("regions_stats-", suffix, ".log")    # output .log 
write.outtxt1 = TRUE                                 # if TRUE, will write merged alignment stats

# variables expected in <stats>
idvar <- "ID"                   # variable name denoting <sample ID>
regvar <- "target"              # variable name in <stats> denoting <region id>
ncontigs <- "ncontigs"          # variable name in <stats> denoting <number of contigs per region>
npassed <- "npassed"            # variable name in <stats> denoting <number of passed contigs per region>
bestscore <- "bestscore"        # variable name in <stats> denoting <raw alignment score of best-matching contig>
bestscoren <- "bestscore.norm"  # variable name denoting <normalized alignment score of best-matching contig>
bestlength <- "bestlength"      # variable name in <stats> denoting <length of best-matching contig>
...
fnames <- c(regvar, ncontigs, bestscore, bestlength) # v1
dc <- read.delim(stats, check.names = FALSE, header = TRUE)
...
if (any(!fnames %in% names(dc))) {
  stop("some expected variables defined in script (lines 115-125) not found in <", stats, ">:\n", 
...
cat(paste0("\nmerged alignment stats have ", nrow(dc), " rows, ", ntaxa, " samples from ", ngroups, " groups (", ifelse(nnsamples > 0, "1", "0"), " thereof set to NA), ", nreg, " regions\n\n"))
...
dc[is.na(dc[,ncontigs]),ncontigs] <- 0 # missing region x sample combinations are assumed to mean zero contigs at that region
...
## Merge data
# merge data (samples in sfile and ranges) with mapfile (matching samples, missing samples will be set to NA)
dmerge <- merge(dc, ds, by = idvar, all.x = TRUE, all.y = FALSE, sort = FALSE)
...
if (is.null(dmerge[,bestscoren])) dmerge[,bestscoren] <- dmerge[,bestscore] / dmerge[,taln]

# add number of contigs (capped numeric variable to plot the heatmap)
dmerge[,Ncont] <- as.numeric(dmerge[,ncontigsc])-1

## Sort dmerge according to groups (needed to create matrix)
varorder <- c(regvar, grpvar1, grpvar2, idvar, ncontigsc, Ncont, ncontigs, bestscore, bestscoren, bestlength, tlen, tgc, taln, tfrac, names(dc)[!names(dc) %in% c(idvar, regvar, ncontigs, bestscore, bestlength, taln, bestscoren)])
dmerge <- dmerge[order(dmerge[,regvar], dmerge[,grpvar1], dmerge[,idvar]),varorder] ; rm(dc)

## Filter samples based on min.pregion (minimum proportion of regions with data)
min.nreg <-  min.pregion * nreg # minimum required number of recovered regions
tax.preg <- tapply(X = dmerge[,npassed], INDEX = dmerge[,idvar], FUN = function(x) {sum(x > 0, na.rm = TRUE)/length(x)})
tax.reg <- tapply(X = dmerge[,npassed], INDEX = dmerge[,idvar], FUN = function(x) {sum(x > 0, na.rm = TRUE)})
tpassed1 <- names(tax.preg[tax.preg >= min.pregion]) # passed samples (sensitivity)
tfailed1 <- names(tax.preg[tax.preg < min.pregion]) # failed samples (sensitivity)
ntpassed1 <- length(tpassed1) # number of passed samples (sensitivity)
ntfailed1 <- length(tfailed1) # number of failed samples (sensitivity)

## Filter samples with low contig length overall (strong fragmentation of loci)
dconlen <- aggregate(dmerge[,bestlength]/dmerge[,tlen], by = list(dmerge[,idvar]), FUN = median, na.rm = TRUE)
tpassed2 <- sort(as.character(subset(dconlen, x >= min.pcontig)[,1])) # passed samples (contig length)
tfailed2 <- sort(as.character(subset(dconlen, x < min.pcontig)[,1])) # failed samples (contig length)
ntpassed2 <- length(tpassed2) # number of passed samples (contig length)
ntfailed2 <- length(tfailed2) # number of failed samples (contig length)

## Define passed groups
tpassed <- unique(sort(intersect(tpassed1, tpassed2)))
tfailed <- unique(sort(c(tfailed1, tfailed2)))
ntpassed <- length(tpassed)
ntfailed <- length(tfailed)

dpreg <- data.frame(dmerge[match(names(tax.preg), table = dmerge[,idvar]),c(idvar, grpvar1, grpvar2)], tax.reg, tax.preg, med.conlen = dconlen$x)
...
cat(paste0(ntpassed1, " (", round(100*ntpassed1/ntaxa,2), "%) samples passed sensitivity filter\n",
           ntpassed2, " (", round(100*ntpassed2/ntaxa,2), "%) samples passed contig length filter\n",
           ntpassed, " (", round(100*ntpassed/ntaxa,2), "%) samples passed both filters\n", 
...
for (group in gcons) {
  ...
  # regions with zero *passed* contigs in > (1-min.ptaxa) of samples per group
  torm1 <- as.character(subset(aggregate(dsub[,npassed], by = list(dsub[,regvar]), FUN = function(x) {sum(x==0)/length(x)}), x > (1-min.ptaxa))[,1])
  lowptaxa <- sort(unique(c(lowptaxa, torm1)))
  
  # regions with ncontigs > max.ncontigs in > (1-min.frac) of samples per group
  torm2 <- find.fails(dsub, ncontigs, regvar, max.ncontigs, "max", min.frac)
  highncontigs <- sort(unique(c(highncontigs, torm2)))
  
  # regions with bestscore.norm < min.bestscore.norm in > (1-min.frac) of samples per group
  torm3 <- find.fails(dsub, bestscoren, regvar, min.bestscore.norm, "min", min.frac)
  lowbestscoren <- sort(unique(c(lowbestscoren, torm3)))
  
  # regions with taln < min.taln in > (1-min.frac) of samples per group
  torm4 <- find.fails(dsub, taln, regvar, min.taln, "min", min.frac)
  lowtaln <- sort(unique(c(lowtaln, torm4)))
  
  # regions with tfrac < min.tfrac in > (1-min.frac) of samples per group
  torm5 <- find.fails(dsub, tfrac, regvar, min.tfrac, "min", min.frac)
  lowtfrac <- sort(unique(c(lowtfrac, torm5)))
  
  # regions with bestscore < min.bestscore in > (1-min.frac) of samples per group
  torm6 <- find.fails(dsub, bestscore, regvar, min.bestscore, "min", min.frac)
  lowbestscore <- sort(unique(c(lowbestscore, torm6)))
  
  # regions with bestlength < min.bestlength in > (1-min.frac) of samples per group
  ...
  regs.passed[[group]] <- all.regs[!all.regs %in% sort(unique(c(torm1, torm2, torm3, torm4, torm5, torm6, torm7)))]
...
cat("\n")
paste1 <- paste0("Regions without aligned contigs in >", 100*(1-min.ptaxa), "% of considered samples in at least 1 group:")
paste2 <- paste0("Regions with >", max.ncontigs, " contigs in >", 100*(1-min.frac), "% as above:")
...
## Write LOG
dlog <- c(
  paste0("=== Filter samples and regions based on visualization of alignment statistics ==="),
  paste0("Starting time: ", t1),
  paste0(""),
  paste0("Input data:"),
  paste0("-----------"),
  paste0("Sample / Grouping file:                               ", sfile),
  paste0("Alignment statistics file:                            ", stats),
  paste0("Reference .fasta:                                     ", refseqs),
  paste0(""),
  paste0("Thresholds for sample filtering:"),
  paste0("-------------------------------"),
  paste0("Min. proportion of regions recovered in a sample:     ", min.pregion),
  paste0("Min. median contig length relative to target length:  ", min.pcontig),
  paste0(""),
  paste0("Thresholds for region filtering:"),
  paste0("--------------------------------"),
  paste0("Min. proportion of samples recovered in a region:     ", min.ptaxa),
  ...
  paste0("Number of PASSED samples (sensitivity filter):        ", ntpassed1, " (", round(100*ntpassed1/ntaxa,2), "%)"),
  paste0("Number of PASSED samples (contig length filter):      ", ntpassed2, " (", round(100*ntpassed2/ntaxa,2), "%)"),
  paste0("Number of PASSED samples (both filters):              ", ntpassed, " (", round(100*ntpassed/ntaxa,2), "%)"),
  ...
  paste0("Output files:"),
  paste0("-------------"),
  paste0("Merged region stats and metadata:                     ", outtxt1, ifelse(write.outtxt1, "", " (not written)")),
...
dpreg$labs <- NA
dpreg$labs[dpreg$passed == 0] <- as.character(dpreg[dpreg$passed == 0,idvar])
p0 <- ggplot(dpreg, aes_string(x = "tax.reg", y = "med.conlen", fill = grpvar3)) +
  geom_density_2d(color = "black", alpha = 0.5, na.rm = TRUE) +
  geom_point(aes_string(color = grpvar3)) +
  geom_text(aes_string(label = "labs"), size = 2, hjust = 0) +
  geom_hline(aes(yintercept = min.pcontig), col = "tomato") +
  geom_vline(aes(xintercept = min.nreg), col = "tomato") +
  geom_vline(aes(xintercept = nreg)) +
  scale_fill_manual(guide = FALSE, values = ycols) +
  scale_colour_manual(guide = FALSE, values = ycols) +
  scale_x_continuous(sec.axis = sec_axis(~./nreg, breaks = min.nreg2lab, labels = min.nreg2lab), limits = c(min.nreg2, nreg)) +
  labs(x = "Number of regions with >= 1 contig per sample", y = "Median contig length relative to region length") +
  facet_wrap(~LABEL) +
  theme_bw() +
...
p1 <- ggplot(dmerge[dmerge[,ncontigs] > 0,], aes_string(x = grpvar3, y = ncontigs, fill = grpvar3)) +
  ...
  coord_flip() +
  theme_bw() +
  ggtitle(paste0("Number of ALL regions: ", nreg, " ; PASSED: ", ncontigs.passed, " (", round(100*ncontigs.passed/nreg,2), "%) ; FAILED: ", nreg-ncontigs.passed, " (", round(100*(nreg-ncontigs.passed)/nreg,2), "%)"))

p2q <- round(quantile(dmerge[,bestscoren], c(qlow, qhigh), na.rm = TRUE), 2)
p2 <- ggplot(dmerge, aes_string(x = grpvar3, y = bestscoren, fill = grpvar3)) +
  geom_boxplot(alpha = 0.5, na.rm = TRUE) +
  geom_violin(alpha = 0.5, na.rm = TRUE) +
  ...
  coord_flip() +
  theme_bw() +
  ggtitle(paste0("Number of ALL regions: ", nreg, " ; PASSED: ", bestscoren.passed, " (", round(100*bestscoren.passed/nreg,2), "%) ; FAILED: ", nreg-bestscoren.passed, " (", round(100*(nreg-bestscoren.passed)/nreg,2), "%)"))

p3q <- round(quantile(dmerge[,bestlength], c(qlow, qhigh), na.rm = TRUE))
p3 <- ggplot(dmerge, aes_string(x = grpvar3, y = bestlength, fill = grpvar3)) +
  geom_boxplot(alpha = 0.5, na.rm = TRUE) +
  geom_violin(alpha = 0.5, na.rm = TRUE) +
  ...
  coord_flip() +
  theme_bw() +
  ggtitle(paste0("Number of ALL regions: ", nreg, " ; PASSED: ", bestlength.passed, " (", round(100*bestlength.passed/nreg,2), "%) ; FAILED: ", nreg-bestlength.passed, " (", round(100*(nreg-bestlength.passed)/nreg,2), "%)"))

p4q <- round(quantile(dmerge[,taln], c(qlow, qhigh), na.rm = TRUE))
p4 <- ggplot(dmerge, aes_string(x = grpvar3, y = taln, fill = grpvar3)) +
  geom_boxplot(alpha = 0.5, na.rm = TRUE) +
  geom_violin(alpha = 0.5, na.rm = TRUE) +
  ...
  coord_flip() +
  theme_bw() +
  ggtitle(paste0("Number of ALL regions: ", nreg, " ; PASSED: ", taln.passed, " (", round(100*taln.passed/nreg,2), "%) ; FAILED: ", nreg-taln.passed, " (", round(100*(nreg-taln.passed)/nreg,2), "%)"))

p5q <- round(quantile(dmerge[,tfrac], c(qlow, qhigh), na.rm = TRUE),2)
p5 <- ggplot(dmerge, aes_string(x = grpvar3, y = tfrac, fill = grpvar3)) +
  geom_boxplot(alpha = 0.5, na.rm = TRUE) +
  geom_violin(alpha = 0.5, na.rm = TRUE) +
  ...
  coord_flip() +
  theme_bw() +
  ggtitle(paste0("Number of ALL regions: ", nreg, " ; PASSED: ", tfrac.passed, " (", round(100*tfrac.passed/nreg,2), "%) ; FAILED: ", nreg-tfrac.passed, " (", round(100*(nreg-tfrac.passed)/nreg,2), "%)"))
...
dref[,ncontigs] <- dcon[match(dref[,regvar], dcon$Group.1),]$x
p7 <- ggplot(dref, aes_string("tlen", "tgc", colour = "PASSED", alpha = ncontigs, size = ncontigs)) +
  ...
  scale_size_continuous(name = "Nb. of\ncontigs") +
...
# PCA 
nvar <- c(ncontigs, bestscoren, bestlength, tlen, tgc, taln, tfrac)
...
# VENN diagram for regions that passed filters in each considered groups (up to 5)
if (draw.venn) {
...
p8 <- suppressWarnings(do.heatmap(dat = dmerge,       xfac = regvar, yfac = idvar, znum = ncontigs,   ygr = grpvar2, ycols = rev(ycols), limit = XandMore,           sortx = sortx,                     hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("ALL regions,", "capped number of contigs")))
suppressWarnings(print(p8))

p9 <- suppressWarnings(do.heatmap(dat = dmerge.keep,  xfac = regvar, yfac = idvar, znum = ncontigs,   ygr = grpvar2, ycols = rev(ycols), limit = XandMore,           sortx = levels(p8$data[,regvar]),  hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("PASSED regions,", "capped number of contigs")))
suppressWarnings(print(p9))

p10 <- suppressWarnings(do.heatmap(dat = dmerge.rm,   xfac = regvar, yfac = idvar, znum = ncontigs,   ygr = grpvar2, ycols = rev(ycols), limit = XandMore,           sortx = levels(p8$data[,regvar]),  hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("FAILED regions,", "capped number of contigs")))
suppressWarnings(print(p10))

# best normalized alignment score (min.bestscore.norm)
p11 <- suppressWarnings(do.heatmap(dat = dmerge,      xfac = regvar, yfac = idvar, znum = bestscoren, ygr = grpvar2, ycols = rev(ycols), limit = p2q[2],             sortx = sortx,                     hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("ALL regions,", "norm. alignment score")))
suppressWarnings(print(p11))

p12 <- suppressWarnings(do.heatmap(dat = dmerge.keep, xfac = regvar, yfac = idvar, znum = bestscoren, ygr = grpvar2, ycols = rev(ycols), limit = p2q[2],             sortx = levels(p11$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("PASSED regions,", "norm. alignment score")))
suppressWarnings(print(p12))

p13 <- suppressWarnings(do.heatmap(dat = dmerge.rm,   xfac = regvar, yfac = idvar, znum = bestscoren, ygr = grpvar2, ycols = rev(ycols), limit = min.bestscore.norm, sortx = levels(p11$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("FAILED regions,", "norm. alignment score")))
suppressWarnings(print(p13))

# best contig length (min.bestlength)
p14 <- suppressWarnings(do.heatmap(dat = dmerge,      xfac = regvar, yfac = idvar, znum = bestlength, ygr = grpvar2, ycols = rev(ycols), limit = p3q[2],             sortx = sortx,                     hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("ALL regions,", "contig length")))
suppressWarnings(print(p14))

p15 <- suppressWarnings(do.heatmap(dat = dmerge.keep, xfac = regvar, yfac = idvar, znum = bestlength, ygr = grpvar2, ycols = rev(ycols), limit = p3q[2],             sortx = levels(p14$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("PASSED regions,", "contig length")))
suppressWarnings(print(p15))

p16 <- suppressWarnings(do.heatmap(dat = dmerge.rm,   xfac = regvar, yfac = idvar, znum = bestlength, ygr = grpvar2, ycols = rev(ycols), limit = min.bestlength,     sortx = levels(p14$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("FAILED regions,", "contig length")))
suppressWarnings(print(p16))

# region alignment length (min.taln)
p17 <- suppressWarnings(do.heatmap(dat = dmerge,      xfac = regvar, yfac = idvar, znum = taln,       ygr = grpvar2, ycols = rev(ycols), limit = p4q[2],             sortx = sortx,                     hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("ALL regions,", "alignment length")))
suppressWarnings(print(p17))

p18 <- suppressWarnings(do.heatmap(dat = dmerge.keep, xfac = regvar, yfac = idvar, znum = taln,       ygr = grpvar2, ycols = rev(ycols), limit = p4q[2],             sortx = levels(p17$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("PASSED regions,", "alignment length")))
suppressWarnings(print(p18))

p19 <- suppressWarnings(do.heatmap(dat = dmerge.rm,   xfac = regvar, yfac = idvar, znum = taln,       ygr = grpvar2, ycols = rev(ycols), limit = min.taln,           sortx = levels(p17$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("FAILED regions,", "alignment length")))
suppressWarnings(print(p19))

# region alignment fraction (min.tfrac)
p20 <- suppressWarnings(do.heatmap(dat = dmerge,      xfac = regvar, yfac = idvar, znum = tfrac,      ygr = grpvar2, ycols = rev(ycols), limit = 1,                  sortx = sortx,                     hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("ALL regions,", "alignment fraction")))
suppressWarnings(print(p20))

p21 <- suppressWarnings(do.heatmap(dat = dmerge.keep, xfac = regvar, yfac = idvar, znum = tfrac,      ygr = grpvar2, ycols = rev(ycols), limit = 1,                  sortx = levels(p20$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("PASSED regions,", "alignment fraction")))
suppressWarnings(print(p21))

p22 <- suppressWarnings(do.heatmap(dat = dmerge.rm,   xfac = regvar, yfac = idvar, znum = tfrac,      ygr = grpvar2, ycols = rev(ycols), limit = 1,                  sortx = levels(p20$data[,regvar]), hclustmethod = hclustmethod, low = low, mid = mid, high = high, title = paste("FAILED regions,", "alignment fraction")))
